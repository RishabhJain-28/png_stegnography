...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self
            .0
            .iter()
            .position(|v| v.chunk_type().to_string() == chunk_type);
        if index.is_none() {
            return Err(anyhow!("chunk type not found"));
        }
        Ok(self.0.remove(index.unwrap()))
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        if value.len() < 8 {
            return Err(anyhow!("Invalid png format, Invalid header length"));
        }
        let header = &value[0..8];
        let valid_header = header
            .iter()
            .zip(Self::STANDARD_HEADER.iter())
            .all(|(a, b)| a == b);

        if valid_header == false {
            return Err(anyhow!("Invalid png format, Invalid header content"));
        }
        ...
        while index < value.len() {
            ...
            let end = index + 12 + len;
            let chunk_bytes = &value[index..(end)];
            let c = Chunk::try_from(chunk_bytes)?;
            chunks.push(c);
            index = end;
        }
...
mod tests {
    ...
    fn test_png_trait_impls() {
        ...
        let png: Result<Png, _> = TryFrom::try_from(bytes.as_ref());
        let png: Png = png.unwrap();
